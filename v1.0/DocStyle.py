...
class DocStyle(DocStyleBase):
    ...
    def uncommented_text_to_lines(self, uncommented_text):
        ...
        param_regex = re.compile(
            "(?P<param_name>.*?)\\s:\\s(?P<param_type>.*?)\n\\s{4}(?P<param_desc>.*)"
        )
        for match in param_regex.finditer(uncommented_text):
            parse_result += (
                "@_hidden_param",
                [
                    match.group("param_name"),
                    match.group("param_type"),
                    match.group("param_desc"),
                ],
            )
        ...
        if match:
            parse_result += (
                "@_hidden_return",
                [match.group("return_type"), match.group("return_desc")],
            )
        ...
        for match in raises_regex.finditer(uncommented_text):
            parse_result += (
                "@_hidden_raises",
                [match.group("raise_type"), match.group("raise_desc")],
            )
